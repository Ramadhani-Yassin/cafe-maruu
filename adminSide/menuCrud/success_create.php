...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $expense_amount = $_POST["expense_amount"] ?? 0.00;
    ...
    if ($check_result->num_rows > 0) {
        ...
        $insert_query = "INSERT INTO menu (item_id, item_name, item_type, item_category, item_price, expense_amount, expense_types, item_description) 
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt = $conn->prepare($insert_query);

        // Bind the parameters
        $stmt->bind_param("ssssddss", $item_id, $item_name, $item_type, $item_category, $item_price, $expense_amount, $expense_types, $item_description);
        ...
        if ($stmt->execute()) {
            ...
            $message = "Error: " . $stmt->error . "<br>SQL: " . $insert_query;
            $iconClass = "fa-times-circle";
            $cardClass = "alert-danger";
            $bgColor = "#FFA7A7"; // Custom background color for error
        }
