...
$item_id = $item_name = $item_type = $item_category = $item_price = $expense_amount = $expense_types = $item_description = "";
$item_id_err = "";
    ...
    if ($stmt = mysqli_prepare($link, $sql)) {
        ...
        if (mysqli_stmt_execute($stmt)) {
            ...
            if (mysqli_num_rows($result) == 1) {
                ...
                $expense_amount = $row['expense_amount'] ?? 0.00;
...
//echo "</pre>";
    ...
    $expense_amount = floatval($_POST["expense_amount"] ?? 0.00);
    $expense_types = isset($_POST["expense_types"]) ? implode(",", $_POST["expense_types"]) : "";
    $item_description = $_POST["item_description"];

    // Update the item in the database
    $update_sql = "UPDATE menu SET item_name='$item_name', item_type='$item_type', item_category='$item_category', item_price='$item_price', expense_amount='$expense_amount', expense_types='$expense_types', item_description='$item_description' WHERE item_id='$item_id'";
...
<body>
    ...
    <form action="" method="post" >
        ...
        <div class="form-group">
            <label for="expense_amount" class="form-label">Incurred Expenses (TZS):</label>
            <input type="number" min="0" step="0.01" name="expense_amount" id="expense_amount" placeholder="0.00" class="form-control" value="<?php echo htmlspecialchars($expense_amount); ?>">
        </div>
        
        <div class="form-group">
            <label for="expense_types" class="form-label">Expense Types:</label>
            <?php 
            $selectedTypes = explode(",", $expense_types);
            $expenseTypeOptions = ['ingredients', 'labor', 'utilities', 'packaging', 'equipment', 'other'];
            ?>
            <?php foreach ($expenseTypeOptions as $type): ?>
                <div class="form-check">
                    <input class="form-check-input" type="checkbox" name="expense_types[]" value="<?= $type ?>" id="expense_<?= $type ?>" <?= in_array($type, $selectedTypes) ? 'checked' : '' ?>>
                    <label class="form-check-label" for="expense_<?= $type ?>"><?= ucfirst($type) ?></label>
                </div>
            <?php endforeach; ?>
        </div>
